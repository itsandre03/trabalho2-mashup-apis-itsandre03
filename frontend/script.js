...
function showPage(pageId) {
    ...
    setTimeout(() => {
        ...
        // Esconder barra lateral em dispositivos móveis após navegação
        ...
        // Se for o painel principal, inicializar a aplicação
...
async function updatePassword(newPassword) {
    try {
        ...
        if (!response.ok) {
            const errorText = await response.text();
            throw new Error(errorText || 'Erro ao atualizar palavra-passe');
        }
        ...
        if (data.success) {
            ...
            throw new Error(data.message || 'Erro ao atualizar palavra-passe');
        }
    } catch (error) {
...
async function getPokemonDetails(name) {
    ...
    try {
        ...
        if (response.status === 401) {
            showAlert('A sua sessão expirou. Por favor, autentique-se novamente', 'danger');
        ...
        if (!response.ok) {
            ...
            throw new Error(errorData.error || 'Erro ao pesquisar Pokémon');
        }
        ...
        pokemonDetails.innerHTML = `
            <div class="alert alert-danger">
                ${error.message || 'Erro ao carregar detalhes do Pokémon. Por favor, tente novamente.'}
            </div>
        `;
    }
}
...
async function getDigimonDetails(name) {
    ...
    try {
        ...
        if (!response.ok) {
            const errorData = await response.json();
            throw new Error(errorData.error || 'Erro ao pesquisar Digimon');
        }
        ...
        digimonDetails.innerHTML = `
            <div class="alert alert-danger">
                ${error.message || 'Erro ao carregar detalhes do Digimon. Por favor, tente novamente.'}
            </div>
        `;
    }
}
...
// Atualizar secção de comparação
...
// Alternar barra lateral em dispositivos móveis
